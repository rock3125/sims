...
bool Application::init() {
    ...
    registry_.emplace<Movement>(sim_entity_, Movement{sim_start, 2.0f, false});
    ...
    switch (ev.type) {
        ...
        case SDL_MOUSEBUTTONDOWN:
            if (ev.button.button == SDL_BUTTON_MIDDLE) {
                ...
                // Live mode: click-to-walk. Set the Sim's movement target.
                int w = 0, h = 0;
                SDL_GL_GetDrawableSize(window_, &w, &h);
                auto ground = camera_.screen_to_ground(ev.button.x, ev.button.y, w, h);
                if (ground && sim_entity_ != entt::null) {
                    MovementSystem::set_target(registry_, sim_entity_, *ground);
                }
            }
            break;
...
void Application::render(double /*alpha*/) {
    ...
    if (ImGui::Begin("Sim Debug")) {
        ImGui::TextUnformatted("Phase 3: Sim avatar + click-to-walk.");
        ImGui::Separator();
